...
<head>
    ...
    <title>Asset Capture Compliance</title>
        ...
        <div class="mx-auto max-w-6xl">
            <div class="no-print mb-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('reports.asset-capture-compliance.pdf', request()->query()) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700">PDF</a>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                <div class="flex flex-wrap items-start justify-between gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h1 class="mt-2 text-2xl font-semibold">Asset Capture Compliance</h1>
                            <p class="mt-1 text-sm text-slate-500">Routines completed but asset capture pending.</p>
                        </div>
                    </div>
                ...
                <div class="no-print mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <form method="GET" class="flex flex-wrap items-end gap-3">
                        ...
                        <a href="{{ route('reports.asset-capture-compliance') }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">Reset</a>
                    </form>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @forelse ($rows as $row)
                                <tr>
                                    ...
                                    <td class="px-4 py-2">{{ $row->performed_at?->format('Y-m-d') }}</td>
                                    <td class="px-4 py-2">{{ $row->performedBy?->name }}</td>
                                    <td class="px-4 py-2">{{ $row->asset_capture_status }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td class="px-4 py-2 text-slate-500" colspan="5">No pending asset captures.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
