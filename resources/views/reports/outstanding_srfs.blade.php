        ...
        <div class="mx-auto max-w-6xl">
            <div class="no-print mb-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('reports.outstanding-srfs.pdf', request()->query()) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700">PDF</a>
                ...
                <div class="flex flex-wrap items-start justify-between gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h1 class="mt-2 text-2xl font-semibold">Outstanding SRFs</h1>
                            <p class="mt-1 text-sm text-slate-500">Missing or draft SRFs with aging.</p>
                        </div>
                    </div>
                ...
                <div class="no-print mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <form method="GET" class="flex flex-wrap items-end gap-3">
                        <div class="flex flex-col">
                            ...
                            <select name="status" class="mt-1 w-40 rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        <div class="flex flex-col">
                            <label class="text-xs font-semibold uppercase tracking-widest text-slate-500">Min Days</label>
                            <input type="number" name="min_days" min="0" value="{{ $minDays }}" class="mt-1 w-24 rounded-md border border-slate-300 px-3 py-2 text-sm" />
                        </div>
                        <button class="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Apply</button>
                        <a href="{{ route('reports.outstanding-srfs') }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">Reset</a>
                    </form>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-semibold uppercase tracking-widest text-slate-500">Created</th>
                                <th class="px-4 py-2 text-left text-xs font-semibold uppercase tracking-widest text-slate-500">Age (days)</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-200">
                            @forelse ($tasks as $task)
                                <tr>
                                    <td class="px-4 py-2">{{ $task->plant?->plant_id }}</td>
                                    <td class="px-4 py-2">{{ $task->plant?->site_name }}</td>
                                    <td class="px-4 py-2">{{ $task->engineer?->name }}</td>
                                    <td class="px-4 py-2">{{ $task->status }}</td>
                                    <td class="px-4 py-2">{{ $task->created_at?->format('Y-m-d') }}</td>
                                    <td class="px-4 py-2">{{ $task->age_days ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td class="px-4 py-2 text-slate-500" colspan="6">No outstanding SRFs.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
